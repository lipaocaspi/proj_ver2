...
class PrivacyPageState extends State<PrivacyPage> {
  ...
  updateUserPassword(id) async {
    final response = await http.put(Uri.parse("http://192.168.1.41:3000/users/$id"),
  ...
  deleteUser(id) async {
    final response = await http.delete(
      Uri.parse("http://192.168.1.41:3000/users/$id"),
      headers: {"Content-Type": "application/json"},
    );
    if (response.statusCode == 200) {
      ...
      if(widget._ridesU.isNotEmpty) {
        for(var i = 0; i == widget._ridesU.length; i++){
          ...
          http.delete(Uri.parse("http://192.168.1.41:3000/rides/$id"));
        }
